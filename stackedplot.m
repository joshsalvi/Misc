...
function figh = stackedplot(z,spacing,style,labels,varargin)
% STACKED PLOT produces a 3D-like plot as a stacked set of spectra. 
% Syntax: figh = stackedplot(z,spacing,style,labels,varargin)
...
%           {0, false, 'none'} - axis tick labels are removed (default).
...
%           {4, true, 'all'} - all axis tick labels are kept.
%       if a 2 element column tick vector, only x-axis tick labels are created.
%       if a 2 element row tick vector, only y-axis tick labels are created.
%       if a 4 element tick vector, then x & y tick labels are created.
%       if a 6 element tick vector, then x,y,&z tick labels are created.
%           An axis tick vector has 2 elements specifying axis min & max values.
...
yvec = 1:zdim(2);
zlimits = [min(z(:)),max(z(:))];
if ischar(labels)||islogical(labels)||numel(labels)==1,
    switch labels,
    ...
    switch numel(labels)
        case 2
            if size(labels,2)==1,       % is true if column vector specifying x-ticks.
                ...
                yflag = false;
                zflag = false;
                [tpts,xvec] = calcticks(zdim(1),labels);
            else
                yflag = true;
                xflag = false;
                zflag = false;
                [tpts,yvec] = calcticks(zdim(2),labels);
            end
        case {4,6}
            xflag = true;
            yflag = true;
            [tpts,xvec] = calcticks(zdim(1),labels(1:2));
            [tpts,yvec] = calcticks(zdim(2),labels(3:4));
            if numel(labels)==4,
                zflag = false;
            else
                zflag = true;
...
switch style
    ...
    case {2,'m','M'}            % do 3D surface mesh plot
        ...
        xflag = yflag;
        yflag = tmpflag;
        axis([yvec(1),yvec(end),xvec(1),xvec(end),zlimits]);
...
%       along the axis. The x and y increment is determined by dividing the total 
...
zrange = max(z(:));
dely = offset(2)*zrange/(ypts-1);
incx = x(2)-x(1);               % calc increment from difference of adj. pts.
delxpts = floor(offset(1)*xpts/(ypts-1));
xall = x(1) + ((1:(xpts+delxpts*(ypts-1)))-1)*incx;
...
for n = 2:ypts,
    yplt = z(:,n)' + dely*(n-1);                % add y offset to selected column
    ytickvec(n) = yplt(end);                    % save y tick location
    ylast(1:delxpts) = 0;                       % set offsetted points along x to zero
    ylast = circshift(ylast,[0,-delxpts]);      % move those points to end
    ylast = max(yplt,ylast);                    % create new ylast
    plot(x+delxpts*(n-1)*incx,ylast,linespec)   % plot ylast
end
hold off
...
function [tickpts,ticklbl] = calcticks(npts,tickvector)
