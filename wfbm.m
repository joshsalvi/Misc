...
%   $Revision: 1.1.6.3 $  $Date: 2007/10/23 19:01:18 $ 
...
if ~isnumeric(H) || H < 0 || H > 1 || isnan(H)
    error('Wavelet:FunctionArgVal:Invalid_ArgVal', ...
        'Fractal index must be a number between 0 and 1');
end
if ~isequal(L,fix(L)) || L < 100 || isnan(L)
    error('Wavelet:FunctionArgVal:Invalid_ArgVal', ...
        'Signal length must be an integer greater than 100');
end
